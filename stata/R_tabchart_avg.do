* restrict sample
    foreach var of varlist isolation depression f1cncpt2 dropout f1_test_avg by_test_avg f1ses male ///
        race hispanic majwht spanish {
    drop if `var' == .
    }
...
** differences in test score average, by isolation
    ...
    * table
        table iso_binary, c(mean f1_test_avg)

     * chart
        graph bar f1_test_avg, over(iso_binary, relabel(1 "Isolation <= 0" 2 "Isolation > 1")) name(isotest, replace) /// 
...
** IV strategy
    table hispanic, c(mean isolation mean f1_test_avg mean dropout)
    table spanish, c(mean isolation mean f1_test_avg mean dropout)
...
* macros for models
    global testchars by_test_avg f1ses male ib4.race
    global dropchars f1_test_avg f1ses male ib4.race
...
* ALL TO HTML 
    ...
    esttab tb db using pbasic_avg.html, replace ///
        b(3) se(3) star(* .1 ** .05 *** .01) nocons nobase compress nogaps scalars(Controls FE) obslast ///
        mtitles("Test" "Dropout") title("Simple OLS") coeflabels(isolation Isolation)

    esttab tc dc using pcontrol_avg.html, replace ///
    ...
    esttab tf df using pfixed_avg.html, replace ///
        b(3) se(3) star(* .1 ** .05 *** .01) nocons nobase compress nogaps keep(isolation) /// 
        scalars(Controls FE) obslast mtitles("Test Avg." "Dropout") title("School Fixed Effects") coeflabels(isolation Isolation) nonotes ///
        addnotes("Standard errors in parentheses." ///
                 "* p<.1, **p<.05, *** p<.01" /// 
   ...
   esttab tb tc tf db dc df using pall_avg.html, replace ///
       b(3) se(3) star(* .1 ** .05 *** .01) nocons nobase compress nogaps scalars(Controls FE) ///
       obslast title("Primary Models") nomtitles mgroups("Test Avg." "Dropout", pattern(1 0 0 1 0 0 )) ///
       order(isolation by_test_avg f1_test_avg f1ses male 1.race 2.race 3.race 5.race) ///
       coeflabels(isolation Isolation by_test_avg "8th Avg." f1_test_avg "10th Avg." f1ses "SES" male "Male" ///
    ...
    esttab tf tfself tfdep df dfself dfdep using palt_avg.html, replace ///
